...
- Add `SyncReadingSessionSchema` + `readingSessions: z.array(...).max(50000).optional().default([])` to `packages/rsvp-core/src/sync.ts`.
...
- Server-side rollups / analytics (this is local-first; the server is dumb storage).</description>
<parentTaskId>task-46</parentTaskId>
<acceptanceCriteria>["`reading_sessions` table exists in capacitor SQLite schema with the columns listed in the description and a Drizzle migration", "Web Postgres schema mirrors the table with a matching migration", "`useReadingSession` (or equivalent) logs sessions from both the in-app RSVP reader and the scroll reader, with pause-on-hidden, periodic heartbeat flush, and noise filtering (<5s or <5 words discarded)", "`SyncReadingSessionSchema` is added to `packages/rsvp-core/src/sync.ts` with a sensible array cap", "Push and pull merge sessions with last-write-wins on `updated_at`", "ESP32 reading does NOT create session rows", "Manual end-to-end test: read a book on two devices signed into the same account; session rows from device A appear on device B after sync"]</acceptanceCriteria>
