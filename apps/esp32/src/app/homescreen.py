...
def draw_homescreen(display, title, progress_pct, wpm):
    """Minimal homescreen. Layout is proportional to display.height."""
    display.clear()
    bg = _bg()
    fg = _fg()
    dim = _dim()
    ...
    display.centered_text(t, H * 5 // 100, color=fg, bg=bg)

    # -- progress bar ----------------------------------------------------
    _draw_bar(display, bar_y, progress_pct)

    # -- stats line ------------------------------------------------------
    stats = f"{progress_pct}%  {wpm} wpm"
    display.centered_text(stats, stats_y, color=dim, bg=bg)

    # -- action hint -----------------------------------------------------
    display.centered_text("press to read", hint_y, color=(180, 180, 180) if not config.INVERSE else (80, 80, 80), bg=bg)
...
def draw_transfer_progress(display, pct, bytes_done, total_bytes):
    ...
    if _last_pct is None:
        display.clear()
        display.centered_text("Receiving", H // 8, color=fg, bg=bg)
        _last_pct = -1
    ...
    # Bar
    _draw_bar(display, bar_y, pct)

    # Labels
    label_y = bar_y + 18
    display.fill_rect(0, label_y, display.width, config.FONT_HEIGHT, bg)
    pct_str = f"{pct}%"
    if total_bytes > 0:
    ...
    display.centered_text(label, label_y + 2, color=dim, bg=bg)
